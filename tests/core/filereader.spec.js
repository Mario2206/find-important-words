const ExternalFileReader = require('../../src/core/external-filereader');
...
describe('FileReader', () => {
  describe('Local Read', () => {
    ...
    it('should start the stream when a path is provided', (done) => {
      ...
      fileReader.setPath(filePath).startStream((value) => {
    ...
    it('should close the stream when the file is fully parsed', (done) => {
      ...
      fileReader.startStream(fakeFunction).then(done);
    });
  });
  ...
  describe('External read', () => {
    ...
    it('should throw an error if the ressource is not found', async () => {
      ...
      try {
        await fileReader.setUri(fakeUrl).startStream(fakeFunction);
      } catch (e) {
        expectedError = e;
      }
